 ...
 * @returns {Object} An object containing {shares, sharesWithSlippage, success}.
 */
function withdraw(token0Amt, token1Amt, token0Decimals, token1Decimals, slippage, dexFee, totalSupplyShares, colReserves) {
    // Adjust token amounts to 12 decimal places for internal calculations
    const token0AmtAdjusted = token0Amt * 10 ** (12 - token0Decimals);
    const token1AmtAdjusted = token1Amt * 10 ** (12 - token1Decimals);

    return helpers._withdrawAdjusted(token0AmtAdjusted, token1AmtAdjusted, slippage, dexFee, totalSupplyShares, colReserves);
}
 ...
 * @returns {Object} An object containing {shares, sharesWithSlippage, success}.
 */
function borrow(token0Amt, token1Amt, token0Decimals, token1Decimals, slippage, dexFee, totalBorrowShares, debtReserves) {
    // Adjust token amounts to 12 decimal places for internal calculations
    const token0AmtAdjusted = token0Amt * 10 ** (12 - token0Decimals);
    const token1AmtAdjusted = token1Amt * 10 ** (12 - token1Decimals);

    return helpers._borrowAdjusted(token0AmtAdjusted, token1AmtAdjusted, slippage, dexFee, totalBorrowShares, debtReserves);
}
